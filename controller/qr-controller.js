...
const qrController = {
    ...
    generateVoucher(req, res) {
        ...
        pool.query(query, [voucherCode, serviceName || null, expirationDate, notes, createdBy], (err, result) => {
            if (err) {
    ...
    getVoucherByCode(req, res) {
        ...
        pool.query(query, [voucherCode], (err, results) => {
            if (err) {
    ...
    redeemVoucher(req, res) {
        ...
        pool.query(checkQuery, [voucherCode], (err, results) => {
            if (err) {
            ...
            if (voucher.status === 'expired' || (voucher.expires_at && new Date() > new Date(voucher.expires_at))) {
                ...
                if (voucher.status !== 'expired') {
                    ...
                    pool.query(updateExpiredQuery, [voucher.voucher_id]);
                }
            ...
            pool.query(redeemQuery, [voucher.voucher_id], (updateErr, updateResult) => {
                if (updateErr) {
    ...
    updateVoucher(req, res) {
        ...
        pool.query(query, [service_name, expires_at, notes, status, voucher_id], (err, result) => {
            if (err) {
    ...
    deleteVoucher(req, res) {
        ...
        pool.query(query, [voucherId], (err, result) => {
            if (err) {
